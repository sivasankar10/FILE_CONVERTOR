...
type JPGtoPNGNavigationProp = NavigationProp<RootStackParamList, 'JPGtoPNG'>;

const JPGtoPNG: React.FC = () => {
  const navigation = useNavigation<JPGtoPNGNavigationProp>();
  ...
  const handleConvert = async () => {
    ...
    try {
      ...
      if (response.status === 200) {
        navigation.navigate('FCDownload', { data });
      } else {
  ...
  return (
    <View style={{ flex: 1, padding: 20 }}>
      ...
      <TouchableOpacity
        ...
        onPress={() => openGallery('JPG_TO_PNG')}
      >
        <Text style={{ color: '#FFF', fontSize: 16 }}>JPG to PNG</Text>
      </TouchableOpacity>
